...
from model.GroupFace import GroupFace
from loss.loss import ArcMarginProduct
from loss.focal_loss import FocalLoss

from dataset.dataset import VGGdataset, totaldata, torch_loader, modifiedvggdata
...
class Trainer():
    ...
    def get_dataloader(self):
        ...
        elif self.cfg['dataset']['name'] == 'modified_vgg':
            from dataset.dataset import totaldata
            dataset = modifiedvggdata(self.cfg['dataset']['image_size'], self.cfg['dataset']['train_path'], self.cfg['dataset']['cache_file'])
        else:
    ...
    def setup_network(self, device):
        model = GroupFace(resnet=self.cfg['model']['num'], feature_dim=self.cfg['option']['feature_dim'], groups=self.cfg['option']['groups']).to(device)
        return model
    ...
    def load_gallery_probe(self):
        gallery_path = self.cfg['dataset']['test_path']
        #
        all_path = []
        p_files = []
        p_ids = []
        ...
        num_thres = self.cfg['solver']['num_thres']
        for i, dir in enumerate(os.listdir(gallery_path)):
            num = 0
    ...
    def training(self):
        ...
        if self.cfg['dataset']['checkpoints_file'] != None:
            self.eval()
    ...
    def eval(self):
        writer = self.t_parameter['writer']
        device = self.t_parameter['device']
        model = self.t_parameter['ema_model'].ema

        model.eval().to(device)

        print('\n[eval] Get gallery and probe images...')

        gallery_files, gallery_ids, probe_files, probe_ids = self.load_gallery_probe()
        ...
        for idxes_batch in tqdm(split_index, total= len(split_index)):
            ...
            for i in idxes_batch:
                gallery_file, _ = self.pad_to_square(torch.Tensor(cv2.imread(gallery_files[i])).permute(2,0,1))
